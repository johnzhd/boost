...
{
	static const std::string private_key("test"); // same as server.csr
	static const std::string chanllenge_key("test_send"); // same as server.csr when it be sent.

	const std::string& key(purpose == boost::asio::ssl::context_base::for_reading ? private_key : chanllenge_key);
...
boost::shared_ptr<boost::asio::ssl::context> http_server_base::new_context(boost::system::error_code & ec)
{
	...
	ptr->use_certificate_chain_file("server.crt", ec);
	if (ec)
		return nullptr;

	ptr->use_private_key_file("server.key", boost::asio::ssl::context::pem, ec);
	if (ec)
		return nullptr;
