...
public:
	get_url_work(boost::shared_ptr<net_thread> net)
		...
		work_loop_function(boost::bind(&get_url_work<T_work_base>::work_loop_enter, this, _1))
	{}
	~get_url_work() {}
public:
	bool start_work()
	{
		//task_urls.push_back("http://support.clean-mx.de/clean-mx/xmlphishing.php");
		task_urls.push_back("https://www.baidu.com?id=1");

		base_core.spawn_start(work_loop_function);
		return true;
	}
	void work_loop_enter(boost::asio::yield_context yc);
	void work_loop_enter_2(boost::asio::yield_context yc);
...
template<typename T_work_base>
inline void get_url_work<T_work_base>::work_loop_enter_2(boost::asio::yield_context yc)
{
	std::vector<char> v;
	v.reserve(18*1024*1024);
	{
		std::ifstream f("1.xml");
		std::string line;
